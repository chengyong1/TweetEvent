...
class similar():
    ...
    def apriori_p(self):
        ...
        reurn p
    def wordSignal(self, k_interval, mode=3):
        data = []
        for key, value in self.data.items():
            data.append(value)
        wordSignalDict = {}
        obj = similarity(data, k_interval)
        if mode==1:
            for word in self.eventWords:
                wordSignalDict[word] = obj.CWTF_ITWTF(word)
        if mode==2:
            for word in self.eventWords:
                wordSignalDict[word] = obj.WF_ITWF(word)
        if mode==3:
            for word in self.eventWords:
                wordSignalDict[word] = obj.WCWTF_ITWCWTF(word)
        return wordSignalDict
    ...
    def combine(self, k_interval, mode=3, verbose=True):
        textSimilarArr = self.textSimilar()
        apriori_pDict = self.apriori_p()
        wordSignalDict = self.wordSignal(k_interval, mode)
        def s1(i, j):
            return textSimilarArr[i][j]
        def s2(i, j):
            sum = 0
            wordsi = self.data[i]
            wordsj = self.data[j]
            for m in range(len(wordsi)):
                for n in range(len(wordsj)):
                    sum += apriori_p[wordsi[m]][wordsj[m]]
            return sum/len(wordsi)/len(wordsj)
        def s3(i, j):
            max = 0
            wordsi = self.data[i]
            wordsj = self.data[j]
            for m in range(len(wordsi)):
                for n in range(len(wordsj)):
                    temp = wordSignalDict[wordsi[m]][wordsj[n]]
                    if temp>max:
                        max = temp
            return max
        def compute(i, j):
            return max(s1(i, j), s2(i, j), s3(i, j))
